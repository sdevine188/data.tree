...
new_data %>% distinct(ben_country_of_birth_groupedMEXIC, BEN_SEXM, BEN_SEXF, weight, weighted_outcome)

# output new_data
write_csv(new_data, path = "output/pre_interview/full/treatment_pre_interview_full_w_tree_output_Case_OutcomeDenied.csv")
...
custom_split_leading_to_node <- c("root_node", "Ben. was married before", "Ben. region of birth is Latin America", 
                                  "Time since present marriage is less than or equal to 7 months",
                                  "Time since present marriage is greater than 7 months",
                                  "Ben. region of birth is not Latin America", "Ben. was not married before",
...
GetNodeLabel <- function(node) {
        ...
        if(current_terminal_node == 1) {
                str_c("Denial rate", "\n", 
                      ...
                      "Non-IBFA: ", percent(node$control_outcome_p_weighted), 
                      " (", percent(node$control_lower_limit_outcome_p_weighted), 
                      "-", percent(node$control_upper_limit_outcome_p_weighted), ")",
                      "\n",
                      "\n",
                      "% of All Petitions", "\n",
                      "IBFA: 100%", "\n", "Non-IBFA: 100%")
                ...
                str_c("Denial rate", "\n", 
                      ...
                      "Non-IBFA: ", percent(node$control_share_p_weighted),
                      " (", percent(node$control_lower_limit_share_p_weighted), 
                      "-", percent(node$control_upper_limit_share_p_weighted), ")")
        }
...
# save plot
tree_plot_title <- str_c("pre_interview_tree_plot_", outcome_variable)
tree_plot_title
